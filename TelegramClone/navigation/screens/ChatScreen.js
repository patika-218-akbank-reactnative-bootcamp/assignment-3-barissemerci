...
const ChatScreen = props => {
  ...
  const {addNewMessages} = useContext(MessagesContext);
  ...
  const renderItem = ({item, index}) => {
    if (item.image) {
      //resimse
      return (
        <View style={item.isMine === 1 ? styles.myImageBox : styles.imageBox}>
      ...
      return (
        <View
          style={item.isMine === 1 ? styles.myMessageBox : styles.messageBox}>
  ...
  return (
    <View style={[styles.container, {backgroundColor: theme.telegramColor}]}>
      <View style={styles.headerStyle}>
        <TouchableOpacity
          onPress={navigateChatsScreen}
          style={styles.backButtonStyle}>
          <IconBack color={'white'} size={40} name="arrow-back" />
        </TouchableOpacity>
        <Image
          style={styles.imageStyle}
          source={{uri: props.route.params.userProfilePhoto}}
        />
        <Text style={styles.textNameStyle}>
          {props.route.params.userName} {props.route.params.userSurname}
        </Text>
        <View style={styles.buttonsStyle}>
          <TouchableOpacity style={styles.callButtonStyle}>
      ...
      <FlatList
        data={props.route.params.userMessages}
        renderItem={renderItem}
        keyExtractor={(item, index) => index}
        style={styles.flatListStyle}
      />
      <InputView id={props.route.params.userId} onPress={addNewMessages} />
    </View>
  );
};
